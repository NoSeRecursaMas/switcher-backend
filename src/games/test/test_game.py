...
def test_skip_turn(client, test_db):
    ...
    db.add_all(players_room_relations)
    db.commit()
    game = GameDB(gameID=1, roomID=room.roomID, board=json.dumps([{"posX": 0, "posY": 0, "color": "R"} for _ in range(36)]),posEnabledToPlay=1)
...
def test_skip_turn_not_player_turn(client, test_db):
    ...
    db.add_all(players_room_relations)
    db.commit()
    game = GameDB(gameID=1, roomID=room.roomID, board=json.dumps([{"posX": 0, "posY": 0, "color": "R"} for _ in range(36)]), posEnabledToPlay=2)
