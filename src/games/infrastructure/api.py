...
from src.games.domain.models import GameID, figureCardID, BoardPiecePosition
...
@router.post(path="/{gameID}/figure", status_code=200)
async def play_figure(gameID: int, figureID: figureCardID, playerID: PlayerID, figure: List[BoardPiecePosition], db_session: Session = Depends(get_db)) -> None:
    game_repository = GameRepository(db_session)
    player_repository = PlayerRepository(db_session)
    room_repository = RoomRepository(db_session)
    ...
    await game_service.play_figure(gameID, playerID.playerID, figureID.cardID, figure)
