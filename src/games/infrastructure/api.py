...
async def delete_partial_movement(gameID: int, playerID: PlayerID, db_session: Session = Depends(get_db)) -> None:
    game_repository = GameRepository(db_session)
    player_repository = PlayerRepository(db_session)

    game_service = GameService(game_repository, player_repository)

    await game_service.delete_partial_movement(gameID, playerID.playerID)