...
class CommonValidators:
    ...
    def validate_no_only_whitespaces(username: str, info: ValidationInfo):
        if not username.isascii():
            raise PydanticCustomError(
                'invalid_length', 'El {value} proporcionado contiene caracteres no permitidos.', {'value': info.field_name})

    @staticmethod
    def verify_whitespaces(value: str, info: ValidationInfo):
        if value.isspace():
            raise PydanticCustomError(
                'invalid_length', 'El {value} proporcionado no puede contener solo espacios en blanco.', {'value': info.field_name})

    @staticmethod
    def verify_whitespace_count(value: str, info: ValidationInfo):
        if " " * 4 in value:
            raise PydanticCustomError(
                'invalid_length', 'El {value} proporcionado no puede contener más de 3 espacios consecutivos.', {'value': info.field_name})
