    ...
    try:
        await manager.broadcast_to_room(
            ...
            message={
                "type": "UPDATE_ROOM",
                "payload": {
                    "msg": f'El jugador "{player_repository.get(playerID).username}" se ha unido a la sala',
                    "status": service.get_public_info(game_id).model_dump(),
                },
            },
        )
        while True:
        ...
        await manager.broadcast_to_room(
            ...
            message={
                "type": "UPDATE_ROOM",
                "payload": {
                    "msg": f'El jugador "{player_repository.get(playerID).username}" ha abandonado la sala',
                    "status": service.get_public_info(game_id).model_dump(),
                },
            },
        )
...
def create_room(room_data: RoomCreationRequest, db_session: Session = Depends(get_db)) -> RoomID:
    service = RoomService(RoomSQLAlchemyRepository(db_session), PlayerSQLAlchemyRepository(db_session))
